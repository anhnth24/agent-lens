//! AgentLens (lean) — theo dõi & review session Claude Code.
//! 1 binary: hook receiver (FR-1) + JSONL tailer (FR-2) + query API + UI server.
//! Store: SQLite (WAL). Zero-token cho capture — chỉ đọc dữ liệu Claude Code đã sinh.
...
async fn main() -> anyhow::Result<()> {
    ...
    let addr = std::env::var("AGENTLENS_ADDR").unwrap_or_else(|_| "127.0.0.1:8787".to_string());
