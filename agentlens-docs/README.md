...
## Bắt đầu code
1. **Capture + Store:** hook receiver (HTTP `:8787`) + JSONL tailer → SQLite (TRD §9 bước 1).
...
Rust 1-binary · SQLite nhúng (WAL) · web UI localhost (không Tauri v1) · LLM 1 provider (Anthropic) tùy chọn · zero-token cho capture · retention 180 ngày.
Còn mở: làm LLM-gợi-ý ngay hay sau; verify thinking-in-JSONL.
